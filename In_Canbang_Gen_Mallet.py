...
for dicit in range(1, 54):

    tlst = []

    notln = random_number2(3, 8)

    print("Voicebuild: ", dicit)

    for adr in range(notln):

        durchprod = random_number2(1, 12)
        durval = 250 * durchprod

        pittchprod = random_number2(-5, 4)
        silch = random_number(8)
        if silch > 1:
            pitval = .083 * pittchprod
            tlst.append((pitval, durval))
        if silch <= 1:
            pitval2 = 'sil'
            tlst.append((pitval2, durval))
...
for segmen in range(seglen):
    ...
    for voc in range(totvoxlen):
        ...
        for pgen in range(pattlen):
            ...
            pauseran = random_number(12)

            if pauseran == 4:
                lensil = random_number2(20000, 80000)
                silaud2 = AudioSegment.silent(duration = lensil)
    ...
    otstr = 'C:\\Users\\mysti\\Coding\\In_C_Gen\\In_Canbang_MixSegment_' + str(tim) + "_" + str(segn) + ".wav"
